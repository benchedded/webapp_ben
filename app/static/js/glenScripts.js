document.addEventListener('DOMContentLoaded', function () {
            ...
            window.calendar = new FullCalendar.Calendar(calendarEl, {
                ...
                eventRender: function(info) {
                    ...
                    // Add context menu for mobile/desktop
                    if (isMobile) {
                        let touchTimer;
                    ...
                    if (info.event.extendedProps.type === 'seizure') {
                        info.el.classList.add('seizure-event');
                ...
                eventClick: function(info) {
                    ...
                    let html = `<p><strong>Date:</strong> ${moment(event.start).format('YYYY-MM-DD')}</p>`;
                    if (event.extendedProps.type === 'medication') {
                        html += `<p><strong>Type:</strong> Medication</p>`;
                        html += `<p><strong>Dosage:</strong> ${event.extendedProps.dosage || 'N/A'}</p>`;
                        html += `<p><strong>Start Date:</strong> ${moment(event.start).format('YYYY-MM-DD')}</p>`;
                        html += `<p><strong>End Date:</strong> ${moment(event.end).format('YYYY-MM-DD')}</p>`;
                        html += `<p><strong>Time to Take:</strong> ${event.extendedProps.time_to_take || 'N/A'}</p>`;
                        html += `<p><strong>Frequency:</strong> ${event.extendedProps.frequency || 'N/A'}</p>`;
                    } else if (event.extendedProps.type === 'seizure') {
                        html += `<p><strong>Type:</strong> Seizure</p>`;
                        html += `<p><strong>Severity:</strong> ${event.extendedProps.severity || 'N/A'}</p>`;
                        html += `<p><strong>Duration:</strong> ${event.extendedProps.duration || 'N/A'}</p>`;
                        html += `<p><strong>Triggers:</strong> ${event.extendedProps.triggers || 'N/A'}</p>`;
                        html += `<p><strong>Notes:</strong> ${event.extendedProps.notes || 'N/A'}</p>`;
                    }

                    bodyEl.innerHTML = html;

                    deleteBtn.onclick = function() {
                        const id = event.id.replace(/^(med_|seizure_)/, '');
